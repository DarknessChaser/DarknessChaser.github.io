...
如果是vue-cli3+直接一套生成的目录，那么理论上不配置也是可以正常使用的，然而有个项目使用用在老版cli生成的，似乎因为这个原因需要单独配置一下。第一个选项更容易一层一层找到翻译文本，第二个选项可以让插件正确读取翻译文件的位置。
```
    "i18n-ally.namespace": true,
    "i18n-ally.pathMatcher": "{locale}/components/{namespace}.json",
```
