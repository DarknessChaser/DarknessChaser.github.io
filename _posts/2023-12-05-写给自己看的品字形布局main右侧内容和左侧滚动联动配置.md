...
这是一种在 2023 年很流行的页面布局方式，bilibili 的视频播放页、知乎的文章页都是类似的设计。类似“品”字的布局大致可以分为以下几部分：
...
当页面主体部分大于一屏高度时，情况会有所不同。

如果是页面主体左侧大于一屏高度，当页面主体右侧滚动到浏览器顶部时会吸顶，并随之滚动。当主体右侧内容滚动到底时，则会吸附在浏览器底部。

如果是页面主体右侧内容大于一屏高度，页面看上去则和正常流式布局类似。
