...
推荐使用 Bundler 锁定依赖版本，避免不同机器上的 Jekyll 版本差异导致行为不一致：

1. `bundle install`
2. `bundle exec jekyll serve`（或 `bundle exec jekyll s`）

然后在浏览器中访问 `http://127.0.0.1:4000/` 预览主题。该模式支持边修改边自动重建（需要刷新浏览器）。
...
如果你收到构建告警，建议先确认本地环境与仓库锁定依赖一致：

1. `bundle install`
2. `bundle exec jekyll serve`

本仓库当前使用 `Gemfile` + `Gemfile.lock` 固定 Jekyll 依赖版本。更新依赖时，请使用 `bundle update` 并提交新的 `Gemfile.lock`。

另外，请确保分页插件使用的是 Jekyll 4 配置写法：`plugins: [jekyll-paginate]`。

> Note：
> 可以使用 `bundle exec jekyll s` 命令在本地实时预览博客，提高效率。

参考文档：[Jekyll Configuration](https://jekyllrb.com/docs/configuration/) & [Bundler Workflow](https://jekyllrb.com/docs/installation/)
